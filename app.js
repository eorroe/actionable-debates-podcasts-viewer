...
const REPO_NAME = 'actionable-debates-podcasts-viewer';
const RAW_BASE = `https://raw.githubusercontent.com/${REPO_OWNER}/${REPO_NAME}/main`;
const TREES_API_BASE = `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/git/trees`;
const CACHE_TTL_MS = 24 * 60 * 60 * 1000;

const REPO_TREE_CACHE_KEY = 'podcasts-repo-tree';
const SOURCES_LIST_CACHE_KEY = 'sources';
const TEACHING_INDEX_CACHE_KEY = 'podcasts-teaching-index';
const QUOTES_CACHE_KEY = 'podcasts-quotes';

const THEME_KEY = 'podcasts-theme';

const PINNED_KEY = 'podcasts-pinned';
const PINNED_ORDER_KEY = 'podcasts-pinned-order';
...
const openCacheDb = () => {
    return new Promise((resolve, reject) => {
        const request = indexedDB.open('podcasts-cache', 1);
            ...
            <div class="flex justify-between items-center mb-4 border-b border-[#c5a059]/30 pb-2">
                <div class="flex items-center gap-2">
                    <button id="filter-sources-btn" class="filter-btn" title="Filter Sources">
                     ...
                     <h2 class="font-['Cinzel'] text-xl">Select Podcast</h2>
                </div>
...
const deleteAllIndexedDb = async () => {
    return new Promise((resolve, reject) => {
        ...
        const request = indexedDB.deleteDatabase('podcasts-cache');
        request.onsuccess = () => resolve();
        request.onerror = (event) => reject(event.target.error);
    });
};
...
const renderRandomQuote = async () => {
    ...
    try {
        ...
        if (!text) {
            const response = await fetch('https://raw.githubusercontent.com/eorroe/actionable-debates-podcasts-viewer/refs/heads/main/quotes.txt?t=' + Date.now());
